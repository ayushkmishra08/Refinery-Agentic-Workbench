...
Entry point: python -m knowledge_layer
Flags:
...
from knowledge_layer.checkpoint import PipelineCheckpoint, PipelinePhase
from knowledge_layer.config import PipelineConfig, load_config
from knowledge_layer.extractor import ExtractionStatus
...
def process_document(
    ...
    if not checkpoint.is_phase_complete(PipelinePhase.PARSING):
        ...
        try:
            from knowledge_layer.parser import DocumentParser
            from knowledge_layer.parse_validation import validate_parsed_document, write_parse_report
    ...
    try:
        ...
        from knowledge_layer.table_reconstructor import reconstruct_table
    ...
    if not checkpoint.is_phase_complete(PipelinePhase.DOCUMENT_PROFILE) or not profile_path.exists():
        ...
        try:
            ...
            from knowledge_layer.schemas.table_schema import TableNormalizationResult
    ...
    if not checkpoint.is_phase_complete(PipelinePhase.GLOSSARY) or not glossary_path.exists():
        ...
        try:
            ...
            from knowledge_layer.schemas.document_profile import DocumentProfile as DP
            if profile is None:
    ...
    try:
        ...
        if not checkpoint.is_phase_complete(PipelinePhase.DOCUMENT_GRAPH):
            ...
            from knowledge_layer.document_graph import write_document_graph
    ...
    if chunks:
        ...
        try:
            from knowledge_layer.embedder import ChunkEmbedder, store_chunks_in_memory
    ...
    if chunks and not checkpoint.is_phase_complete(PipelinePhase.EXTRACTION):
        ...
        if no_llm:
            ...
            try:
                from knowledge_layer.extractor import OllamaExtractor
        ...
        from knowledge_layer.schemas.document_profile import DocumentProfile as DP
        from knowledge_layer.schemas.knowledge import ChunkExtraction
        from knowledge_layer.entity_resolver import EntityResolver
        from knowledge_layer.graph import GraphInserter
        from knowledge_layer.ontology_manager import OntologyManager
        from knowledge_layer.retriever import MemoryRetriever, RetrievalContext
        from knowledge_layer.validator import ExtractionValidator
    ...
    try:
        from knowledge_layer.reporter import ReportGenerator
        from knowledge_layer.reference_tracker import ReferenceTracker
