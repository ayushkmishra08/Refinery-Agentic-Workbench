...
| File | Purpose |
|------|---------|
| [`config.py`](file:///c:/Users/pandi/Documents/AyushCodes/Refinery_KL_Extraction/src/config.py) | Central configuration with VRAM-safe defaults |
...
| [`extractor.py`](file:///c:/Users/pandi/Documents/AyushCodes/Refinery_KL_Extraction/src/extractor.py) | Constrained LLM extraction via Ollama |
...
When you run python -m src, the pipeline starts up and acts as an orchestrator for processing PDF documents. Here is the exact sequence of events:
...
Resilient Checkpointing: Because of PipelineCheckpoint, if the model or pipeline does crash midway through Phase 8, you can just rerun python -m src. The system will automatically skip all completed phases and already-extracted chunks, resuming exactly where it left off.
