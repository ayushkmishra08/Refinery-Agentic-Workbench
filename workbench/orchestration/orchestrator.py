...
class Orchestrator:
    ...
    def _run(self, rs: RunState, request: UserRequest, on_event=None) -> FinalResponse:
        ...
        try:
            ...
            phases.append(AuditPhase(name="retrieval", agent="context_builder", status="done", duration_ms=int((time.time() - t0) * 1000),
                                     note=f"route={context.route}; claims={len(context.claims)} relations={len(context.relations)} procedures={len(context.procedures)} chunks={len(context.chunks)}"))
            events.emit("phase_finished", phase="2 Specialist retrieval", message=phases[-1].note, data={"route": context.route, "gaps": context.gaps})
            ...
            phases.append(AuditPhase(name="planning", agent="planner", status="done", duration_ms=int((time.time() - t0) * 1000), note=plan.rationale))
            events.emit("plan_created", phase="3 Planning", message=plan.rationale, data={"steps": [{"id": s.step_id, "agent": s.agent, "goal": s.goal, "depends_on": s.depends_on} for s in plan.steps]})
            ...
            phases.append(AuditPhase(name="execution", agent="executor", status="done", duration_ms=int((time.time() - t0) * 1000),
                                     note=f"{sum(1 for s in plan.steps if s.status == StepStatus.DONE)}/{len(plan.steps)} steps done" + (f", {iteration} replan(s)" if iteration else "")))
            ...
            events.emit("agent_finished", phase="5 Governance", agent="verification", step_id="verify", message=ver.summary, data={"ok": True})
            ...
            phases.append(AuditPhase(name="governance", agent="governance", status="done", duration_ms=int((time.time() - t0) * 1000), note=f"status={resp.status}; confidence={resp.confidence.score}; review={resp.requires_human_review}"))
